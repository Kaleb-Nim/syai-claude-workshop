...
Phase 2 teaches the Skills primitive via a 5-section chapter + 1 build-gate plan. CONTEXT.md
...
| Section assembly | `pages/02-skills.md` orchestrator | — | Mirrors Phases 1/2 — list of `src:` blocks |
| Per-section slide content | `pages/02-skills/0N-*.md` files | — | One file per section; atomic commit per file (QUAL-02) |
...
| Hands-on artifact | Participant's local `~/.claude/skills/pirate-mode/SKILL.md` | — | NOT a repo file; same pattern as Phase 3's `~/.claude/settings.json` |
...
- **D-11..D-14:** Slidev conventions carried forward from Phase 3.

### Claude's Discretion (planner's freedom)

- Section 01 cover-slide one-liner (presenter-flavored provocation).
- Exact wording of Phase 3 → 3 bridge on Section 02 Slide 1.
...
| QUAL-02 | Atomic per-section commits | One commit per section file; conventional-commit prefix matches Phase 1/2 (`feat(phase-2): section 0N <name>`) |
...
**No new installs required for Phase 2.** Everything is already wired.
...
slides.md  (root orchestrator, head matter LOCKED)
    ...
    ├── src: ./pages/03-hooks.md           (existing — Phase 3)
    └── src: ./pages/02-skills.md          (NEW — Phase 2 adds this line)
                │
                ├── src: ./02-skills/01-cover.md
                ├── src: ./02-skills/02-why-skills.md
                ├── src: ./02-skills/03-how-skills-work.md
                ├── src: ./02-skills/04-hands-on-build.md
                └── src: ./02-skills/05-hands-on-fire.md
                          │
...
```
pages/
├── 02-skills.md              # NEW — chapter orchestrator (5 src: includes)
└── 02-skills/                # NEW directory
...
### Pattern 1: Chapter Orchestrator (`pages/02-skills.md`)
...
**Example (matches Phase 3 cover verbatim):**
```markdown
---
layout: center
---
...
```
Source: `pages/03-hooks/01-cover.md`.
...
```
Source: convention used across `pages/01-audience-setup/0N-*.md` and Phase 3.
...
class. **Phase 2 Section 05 MUST use this exact pattern** — two `✓ Check:` blockquotes,
...
hit that overflow threshold and shipped with a `<style scoped>` size tweak — Phase 2
planner should compose the slide first, then decide.

### Anti-Patterns to Avoid

- **Adding new fields to `slides.md` head matter.** It's locked. Phase 2 appends
...
- **Putting cross-OS shell command swaps inside the fenced ```markdown``` block.** The
  SKILL.md content is portable Markdown — there's no OS variance here (unlike Phase 3's
  `afplay` / `aplay` split). Don't invent platform splits where none exist.
- **JSON comments inside a `json` fenced block** — N/A for Phase 2 (no JSON taught) but
  worth noting the planner doesn't need Phase 3's comment-discipline rule here.
...
| `✓ Check` styled component | Custom Vue component | Plain `> ✓ Check:` blockquote | Phase 1 + Phase 3 use plain markdown |
| Skill triggering on the slide | Try to demo skill from inside slidev | Participants run `claude` in their own terminal — same pattern as Phase 3 | Phase 3's hands-on already runs outside the slide |
...
a few scoped `<style>` blocks. Phase 2 follows suit — there is no Phase 2-specific
...
**Why it happens:** Phase 3 already established the policy: native Windows
participants run Claude Code inside WSL. The Phase 3 footnote remains valid.
**How to avoid:** Add a short footnote on the Section 04 paste slide mirroring Phase 3
Section 03's: *"Windows — run inside WSL."* Or rely on the global Phase 3 policy and
omit, since Phase 2 inherits the audience setup.
**Warning signs:** Participant reports `'mkdir' is not recognized`.
**Source:** Phase 3 D-06 cross-OS note; verified by inspection of Phase 3 CONTEXT.md.
...
Not applicable — Phase 2 is a content-creation phase. No renames, refactors, or
migrations of existing state. New files are additive:
- `pages/02-skills.md` (new)
- `pages/02-skills/0N-*.md` (5 new files)
...
- No Phase 2 content depends on deprecated APIs.
...
| Playwright Chromium | NOT required by Phase 2 (only QUAL-03 PDF export, deferred to Phase 5) | n/a for this phase | — | — |
| Claude Code (participant-side) | Section 05 hands-on | participant-supplied | — | — (workshop prereq; covered by Phase 1) |
| `mkdir`, plaintext editor | Section 04 hands-on | macOS/Linux/WSL ✓; native Windows ✗ on `-p` | — | Windows → WSL (Phase 3 policy) |

**Missing dependencies with no fallback:** None for the deck build. Section 04's
`mkdir -p` is the only platform-sensitive command — Phase 3's WSL policy covers it.

**Missing dependencies with fallback:** None requiring fallback in the repo. Participant
machines outside macOS/Linux/WSL are routed to WSL per Phase 3.

## Validation Architecture

`workflow.nyquist_validation` is not set in `.planning/config.json` → enabled by
default. However, Phase 2 is a content-authoring phase with **no automated test
...
| QUAL-02 | One commit per section file | automated | `git log --oneline -- pages/02-skills/0N-*.md` (expect exactly 1 commit per file) | ✓ git history |
| QUAL-04 | Tone — no exclamation marks in slide prose | semi-automated | `grep -n '!' pages/02-skills/*.md` (manual review; exclude SKILL.md content quotes) | ✓ grep |

### Sampling Rate
- **Per task commit:** save → Slidev dev server auto-reloads → eye check
- **Per wave merge:** N/A (Phase 2 is sequential — one section at a time)
...
`security_enforcement` not explicitly set; default = enabled. However, Phase 2 has
...
| Build-time dependency vulnerability | Tampering | Bun lockfile already pinned; out of Phase 2 scope |

**No new security controls are required for Phase 2.**

## Project Constraints (from CLAUDE.md)

These directives MUST be honored by all Phase 2 work:
...
- **Atomic commits:** One commit per section (`feat(phase-2): section 0N <name>`).
...
- **MacOS-local-friendly tooling:** Anything heavy is fine to recommend (M4 Max 128GB)
  but Phase 2 is content-only, no local-model usage.
...
- This repo: `pages/01-audience-setup.md` + `pages/01-audience-setup/0N-*.md` (Phase 1
  conventions), `pages/03-hooks.md` + `pages/03-hooks/01-cover.md` (Phase 3 cover
...
- Architecture: HIGH — Phase 2 mirrors Phase 1 + Phase 3 exactly.
- Pitfalls: HIGH — Pitfall 1 (hot-reload) is the one drift item; all others are
  inherited from Phase 3 + verified.
...
hot-reload behavior is recent; revisit before next phase ships if Phase 2 doesn't
ship within 30 days).
...
### File Created
`/Users/kalebnim/Documents/GitHub/syai-claude-workshop/.planning/phases/02-skills/03-RESEARCH.md`
