...
- `disable-model-invocation`, `user-invocable`, `allowed-tools`, `paths`, `context: fork`, `agent`, `model` frontmatter fields — listed by name only on Section 03's frontmatter slide ("more fields exist — `name` + `description` is enough today"), not taught. (Section 03 mirrors Phase 2 D-05 pattern for non-taught events.)
...
- Project-level `.claude/skills/` — covered by mention only in Section 03 ("user-level today; project-level versions with the repo"). Hands-on is user-level only, mirroring Phase 2's `~/.claude/settings.json` pattern.
...
- **D-01:** Chapter 3 splits into 5 content sections + 1 closing build plan = **6 plans** for `/gsd-plan-phase 3`:
  - `pages/03-skills/01-cover.md` — chapter cover slide (one-line provocation).
  - `pages/03-skills/02-why-skills.md` — model-invoked framing + Phase 2 bridge + 3 locked verbatim quotes (SKL-01 framing half).
  - `pages/03-skills/03-how-skills-work.md` — `.claude/skills/<name>/SKILL.md` file shape + minimal frontmatter (`name` + `description`) + 3-tier progressive disclosure (SKL-01 mechanism half + SKL-02).
  - `pages/03-skills/04-hands-on-build.md` — participants `mkdir -p ~/.claude/skills/pirate-mode/` and paste a minimal SKILL.md (SKL-03 wire).
  - `pages/03-skills/05-hands-on-fire.md` — participants prompt Claude in plain English → skill auto-invokes → ✓ Check on the distinctive signature (SKL-03 fire).
  - **Plan 06:** `bunx slidev build` + presenter spot-check (QUAL-01).
- Mirrors Phase 2's 6-plan chapter shape (Phase 2 D-01) and Phase 1's chapter-orchestrator pattern.
- Atomic per-section commits per QUAL-02.

### Section 02 — Why skills (D-02, D-03, D-04)
- **D-02 (Section 02 shape):** 5 slides total. Model-invoked framing first (slides 1–2), then the 3 locked verbatim quotes (slides 3–5). Mirrors Phase 2 D-02 shape.
  - Slide 1 — The wow framing: in Chapter 2 you wired a hook — *you* told Claude exactly when to run something. A skill is the opposite end: *Claude* decides when to run it. (Phase 2 → 3 bridge per D-04.)
...
- **D-04 (Phase 2 bridge on Slide 1):** Slide 1 of Section 02 MUST include an explicit callback to Phase 2's hook hands-on. Frame: *hook = you wire the exact "when" (deterministic THEN); skill = Claude decides the "when" (model-invoked HOW)*. Wording is at planner discretion but the bridge MUST be present. Reason: continuity with Chapter 2's last act + the model-invoked distinction is the load-bearing teaching, and contrast against hooks makes it land cleanly.
...
- **D-08 (paste shape):** Participants paste the EXACT SKILL.md from D-06 into a single new file. Two-step paste:
  ...
  Slide includes one shell block (the `mkdir`) and one fenced ```markdown``` block (the SKILL.md content). Plain-text one-liner above the SKILL.md block: *"This is the same SKILL.md from the last slide — paste it as-is."* (Reduces "is this different?" friction; mirrors Phase 2 D-08 reconcile-note pattern.)
...
- **D-10 (trigger + ✓ check):** Two micro-steps inline on one slide:
  ...
  - Closing line: *"Two prompts, one skill. Claude chose when to use it — that's the whole point."* (closes the chapter on the model-invoked distinction rather than a new concept; mirrors Phase 2 D-09 closing-line shape.)
- ✓ Check style follows the Phase 1 D-08 / Phase 2 D-09 pattern (terminal-style or callout-style fenced block — exact rendering at planner/researcher discretion, but consistent with Phases 1–2).

### Slide-level shape carried forward from Phase 1 + Phase 2 (D-11..D-14)
- **D-11:** Section files use Slidev's per-slide `---` separator with minimal per-slide frontmatter. No `layout: section` slides at section boundaries within the chapter. (Carries Phase 2 D-10.)
- **D-12:** Code blocks ≤25 lines per slide. SKILL.md bodies in fenced ```markdown``` (or whichever highlighter renders frontmatter cleanest); shell snippets in ```bash``` blocks. No custom `<Cmd>` Vue component. (Carries Phase 2 D-11.)
- **D-13:** `<v-clicks>` used sparingly — only on Section 02 quote-reveal if pacing genuinely benefits, and on neither of the hands-on sections (they're already short). (Carries Phase 2 D-12.)
- **D-14:** Tone discipline (QUAL-04): confident, terse, no hype, no exclamation marks in body, no screenshots. Carries Phase 2 D-13. **Exception**: the brain-rot vocabulary inside the SKILL.md body (D-06) is deliberate content of the example skill and does NOT count as hype — it's the demo payload. Slide prose around it stays on-tone.

### Claude's Discretion
- Exact cover-slide one-liner (Section 01) — tight provocation; presenter-flavored.
- Exact wording of the Phase 2 callback line on Section 02 Slide 1 (D-04) — must contrast hook (you wire the "when") vs skill (Claude decides the "when"); exact phrasing open.
...
- `.planning/REQUIREMENTS.md` — Phase 3 requirements: SKL-01, SKL-02, SKL-03, QUAL-01, QUAL-02, QUAL-04.
- `.planning/ROADMAP.md` — Phase 3 goal + dependencies (depends on Phase 2) + per-phase research policy.
- `slides.md` — existing chapter orchestrator (preserve headmatter; add `src:` include for `pages/03-skills.md` AFTER the existing Phase 1 + Phase 2 includes).

### Phase 1 + Phase 2 artifacts (direct dependencies)
- `.planning/phases/01-audience-setup/01-CONTEXT.md` — chapter-orchestrator pattern, ✓ check shape, tone discipline, slide-level conventions D-14..D-17 (carried into Phase 2 D-10..D-13 → Phase 3 D-11..D-14).
- `.planning/phases/01-audience-setup/01-08-PLAN.md` — Phase 1 closing `bunx slidev build` plan; Phase 3's plan 06 mirrors its shape.
- `.planning/phases/02-hooks/02-CONTEXT.md` — chapter shape locked here (6 plans, locked-quotes pattern, IFTTT framing → quote reveals → hands-on build → hands-on fire). Phase 3 D-01..D-14 mirror Phase 2 D-01..D-13 with Skills-specific substitutions.
- `pages/01-audience-setup.md` and `pages/01-audience-setup/*.md` — Slidev frontmatter conventions, layout choices, ✓ check rendering. Phase 3 must visually match.
- `pages/02-hooks.md` and `pages/02-hooks/*.md` (once Phase 2 ships) — most-recent chapter-orchestrator + section-file reference; Phase 3 mirrors this exactly. **If Phase 2 has NOT shipped at Phase 3 plan time**, planner falls back to Phase 1's files and matches Phase 2's CONTEXT.md `D-10..D-13` conventions on faith.
...
**Both sources reviewed during /gsd-discuss-phase 3 and informed D-02..D-10. Researcher MUST re-fetch at plan time to catch any drift and to mine any sections this CONTEXT.md didn't pull through.**
...
### Reusable Assets
- `slides.md` — has v1.0 head matter (theme, fonts, Shiki, colorSchema) and `src:` includes for Phase 1 (Phase 2 will add another). Phase 3 appends one more `src:` line for `pages/03-skills.md`. Does NOT modify head matter.
- `pages/01-audience-setup.md` + `pages/02-hooks.md` (once shipped) — chapter orchestrator templates Phase 3 mirrors at `pages/03-skills.md`.
- `pages/01-audience-setup/0N-*.md` + `pages/02-hooks/0N-*.md` (once shipped) — section file templates Phase 3 mirrors at `pages/03-skills/0N-*.md` (same frontmatter conventions, slide-separator style, ✓ Check rendering).
- `style.css`, `uno.config.ts` — v1.0 theme tokens. Phase 3 sections use these only; do NOT add new colors. All custom CSS scoped under `.slidev-layout`.
- `components/global-top.vue` — 3px rust progress bar already wired. Phase 3 doesn't touch it; it Just Works.
- `.claude/skills/higgsfield-*` (project-local) — existing skill folders on disk. NOT used as the workshop example (Phase 3's example is a fresh `pirate-mode` skill at user-level), but their existence confirms the file-shape Section 03 teaches matches what's already in the repo. Researcher MAY peek at one of these as a sanity check that the frontmatter shape taught is current.

### Established Patterns
- **Chapter-file pattern (locked):** `slides.md` → `pages/03-skills.md` → `pages/03-skills/0N-*.md`. Identical to Phases 1 + 2.
- **Atomic section commits (QUAL-02):** one git commit per section file, scoped commit message (`feat(phase-3): section 02 why-skills`, etc.).
- **Tone discipline (QUAL-04):** confident + terse + no hype + no exclamation marks in body + no screenshots. v1.0 D-23 / Phase 1 D-17 / Phase 2 D-13 standard. (Brain-rot vocabulary inside the example SKILL.md body is content of the demo skill, NOT slide prose — see D-14 exception.)
- **✓ Check pattern:** Phase 1 Sections 05/06 + Phase 2 Section 05 use a copy-paste-compare check on a callout-style block. Phase 3 Section 05 mirrors this shape (text-signature check rather than audio-cue or visual-output but same callout style).
- **Locked-quotes pattern:** Phase 2 D-03 introduced the "3 verbatim quotes, no rewording" pattern. Phase 3 D-03 carries it forward with 3 different (skills-specific) quotes.

### Integration Points
- `slides.md` — Phase 3 adds a single `---\nsrc: ./pages/03-skills.md\n---` block AFTER the existing Phase 1 (and Phase 2, once shipped) includes. Cover slide stays untouched.
- `pages/03-skills.md` — NEW file; chapter orchestrator with `src:` includes for each section file.
- `pages/03-skills/` — NEW directory; 5 section files (`01-cover.md` through `05-hands-on-fire.md`).
- `~/.claude/skills/pirate-mode/SKILL.md` — NOT a repo file. Participants create this on their own machines during the hands-on; no repo change. (Same pattern as Phase 2's `~/.claude/settings.json` edit.)
...
- **SKL-01 framing (user-approved):** model-invoked vs user-invoked is the wow framing. Bridge from Phase 2 (you wired a hook = exact "when"; skill = Claude decides "when") makes the distinction concrete. ELI5 tone — no jargon up front. The 3 locked quotes carry the load-bearing teaching.
...
- **Phase 2 bridge:** Slide 1 of Section 02 must include the hook-vs-skill contrast — hook = you wire the deterministic "when"; skill = Claude decides the "when." Bridges to Phase 2's hands-on directly.
