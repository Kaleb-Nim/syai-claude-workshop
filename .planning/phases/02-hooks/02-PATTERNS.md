# Phase 2: Hooks — Pattern Map

**Mapped:** 2026-05-12
**Files analyzed:** 7 (1 modify + 6 new)
**Analogs found:** 7 / 7 (every Phase 2 file has an exact Phase 1 analog now shipped under `pages/01-audience-setup/`)
...
| `pages/02-hooks.md` (NEW) | orchestrator (chapter) | static-include | `pages/01-audience-setup.md` (full file, 21 lines) | exact |
| `pages/02-hooks/01-cover.md` (NEW) | section (cover) | static-render | `pages/01-audience-setup/01-cover.md` | exact |
| `pages/02-hooks/02-why-hooks.md` (NEW) | section (multi-slide prose + blockquotes) | static-render | `pages/01-audience-setup/02-prereqs-install.md` (multi-slide via `---`) + `07-win-claude-md.md` (blockquote framing) | exact (multi-slide) |
| `pages/02-hooks/03-how-hooks-work.md` (NEW) | section (single slide, fenced JSON + dim notes) | static-render | `pages/01-audience-setup/06-win-statusline.md` (single-slide demo with fenced blocks) | exact (single-slide demo) |
| `pages/02-hooks/04-hands-on-build.md` (NEW) | section (single slide, larger fenced JSON) | static-render | `pages/01-audience-setup/05-claude-files-primer.md` (large fenced block + optional `<style scoped>` shrink) | exact (large-block variant) |
| `pages/02-hooks/05-hands-on-fire.md` (NEW) | section (steps + two ✓ Check blockquotes) | static-render | `pages/01-audience-setup/06-win-statusline.md` + `07-win-claude-md.md` (✓ Check shape) | exact |

**Note on analog quality:** Unlike Phase 1 (greenfield, no `pages/` existed), Phase 2 has direct, shipped, byte-exact analogs for every file. Planner should **copy-paste Phase 1 file shapes and substitute Hook content**.
...
### `pages/02-hooks.md` (orchestrator, static-include)
...
**Phase 2 whole-file pattern** (substitute paths; 5 includes instead of 7):
```markdown
---
...
- **Path is relative to `pages/`** (the importing file's directory): `./02-hooks/01-cover.md`, NOT `./pages/02-hooks/01-cover.md`. (Phase 1 Pitfall 3, still applies.)
- **Trailing newline after the final `---`.** Phase 1 file ends with `---\n`.

---

### `pages/02-hooks/01-cover.md` (section, cover)
...
- **H1:** `# Chapter N — <Title>` (em-dash, not hyphen). Phase 2 → `# Chapter 2 — Hooks`.
- **One-line provocation:** plain lowercase prose immediately under H1. No `!`. (Planner's discretion per CONTEXT.)
- **Dim subtitle div:** `<div class="text-sm opacity-60 mt-8">` with content using ` · ` (middle-dot) as separator. Phase 2 candidate: `why hooks · how they work · one chime you'll wire live`.
- **No trailing whitespace.** File ends with the closing `</div>` line + newline.

---

### `pages/02-hooks/02-why-hooks.md` (section, multi-slide prose + blockquotes)
...
**Phase 2 Section 02 shape (5 slides, locked content per D-02/D-03):**
...
### `pages/02-hooks/03-how-hooks-work.md` (section, single-slide demo)
...
### `pages/02-hooks/04-hands-on-build.md` (section, single-slide large-block paste)
...
**When to apply:** Only if the rendered block visually overflows at presenter resolution. Phase 1 used this for a 27-line file tree. Phase 2 Section 04's JSON is ~22 lines — **draft WITHOUT first**, retrofit only at Plan 06 spot-check (RESEARCH Open Question 3).
...
### `pages/02-hooks/05-hands-on-fire.md` (section, steps + ✓ Check blockquotes)
...
**Apply to:** All Phase 2 section files.
...
**Apply to:** All fenced blocks in Phase 2.
...
**No Shiki annotations used in Phase 1:** No `{1,3}` line-highlighting, no `[!code highlight]`, no `[!code ++]`/`[!code --]`. Phase 2 follows suit — bare fence + language tag only.
...
**Apply to:** Every Phase 2 section file body.
...
None. Every Phase 2 file has a direct, shipped, byte-exact Phase 1 analog. RESEARCH confirms no new components, no new CSS, no new patterns required.

| File | Status |
|------|--------|
| All 7 Phase 2 files | Direct analog in `pages/01-audience-setup/` |
...
10. **Nested `src:` paths are relative to importing file's directory** (Phase 1 Pitfall 3). `./02-hooks/01-cover.md`, NOT `./pages/02-hooks/01-cover.md`.
