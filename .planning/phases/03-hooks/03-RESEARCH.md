...
**Section breakdown (D-01):** Chapter 3 = 5 content sections + 1 closing build plan = 6 plans.
- `pages/03-hooks/01-cover.md` — chapter cover (one-line provocation).
- `pages/03-hooks/02-why-hooks.md` — IFTTT framing + 3 locked determinism quotes (HOOK-01).
- `pages/03-hooks/03-how-hooks-work.md` — Stop + Notification events + minimal `settings.json` JSON shape (HOOK-02).
- `pages/03-hooks/04-hands-on-build.md` — participants paste Stop+Notification JSON into `~/.claude/settings.json` (HOOK-03 wire).
- `pages/03-hooks/05-hands-on-fire.md` — trigger Claude, hear chimes, ✓ check (HOOK-03 fire).
...
Phase 3 is a 5-section authoring phase plus build/spot-check closer. The visual/structural machinery is **fully solved** by Phase 1 — every pattern (chapter orchestrator, nested `src:`, section file frontmatter, ✓ Check blockquote, Shiki code block with auto-styled tinted panel + rust left rule, tone discipline) is locked. The Phase 3 plan should treat Phase 1's `01-PATTERNS.md` and existing section files as the template and **copy-paste** their structure.
...
|------------|-------------|----------------|-----------|
| Chapter orchestration (`slides.md` → `pages/03-hooks.md`) | Slidev parser (build time) | — | Single new `src:` block appended to `slides.md`; mirrors Phase 1 add. |
...
|------------|-----------|----------|
| Nested `src:` chapter orchestrator | Inline all 5 sections in `pages/03-hooks.md` | Loses atomic per-section commit clarity (QUAL-02). Same conclusion as Phase 1. |
...
slides.md (locked headmatter: theme/fonts/Shiki/colorSchema — DO NOT TOUCH)
  ...
  │
  └── src: ./pages/03-hooks.md           (NEW — appended at EOF)
            │
            └── pages/03-hooks.md  (chapter orchestrator — NEW, pure src: includes)
                  │
                  ├─── src: ./03-hooks/01-cover.md             ──► 1 slide (chapter cover)
                  ├─── src: ./03-hooks/02-why-hooks.md         ──► 5 slides (IFTTT + 3 quotes)
                  ├─── src: ./03-hooks/03-how-hooks-work.md    ──► 1 slide (events + JSON)
                  ├─── src: ./03-hooks/04-hands-on-build.md    ──► 1 slide (paste JSON)
                  └─── src: ./03-hooks/05-hands-on-fire.md     ──► 1 slide (trigger + ✓ checks)
                  ▼
...
├── 03-hooks.md                     # NEW — chapter orchestrator (pure src: includes)
└── 03-hooks/                       # NEW directory
    ├── 01-cover.md
    ├── 02-why-hooks.md
...
**Example — `pages/03-hooks.md` (complete file, 15 lines):**
```markdown
---
...
**Path resolution rule (Phase 1 Pitfall 3, still applies):** Inside `pages/03-hooks.md`, use `src: ./03-hooks/01-cover.md` (relative to `pages/`). NOT `src: ./pages/03-hooks/01-cover.md`.
...
**File structure (`pages/03-hooks/02-why-hooks.md`):** 5 slides separated by `---` mid-file, no per-slide frontmatter (default layout).
...
**File structure (`pages/03-hooks/03-how-hooks-work.md`):** One slide, no frontmatter.
...
**File structure (`pages/03-hooks/04-hands-on-build.md`):** One slide, no frontmatter.
...
**File structure (`pages/03-hooks/05-hands-on-fire.md`):** One slide, no frontmatter.
...
- **Do NOT add `layout: section` inside Chapter 3** (same as Phase 1 D-14).
...
| Terminal/JSON-style code block | Custom `<Cmd>` Vue component | Plain ` ```bash `/` ```json ` + Phase 3 CSS contract (already wired) | AUTH-01 deferred to v1.2. |
...
**Key insight:** Phase 3 is content authoring on a Phase 1-solved chassis. Every mechanism (theme, fonts, code-block styling, progress bar, `src:` orchestration, ✓ Check, click animation) already exists. **Resist any temptation to introduce new components, new CSS, or new patterns.**
...
**How to avoid:** The Phase 3 edit to `slides.md` appends exactly one block at the end of the file:
```markdown
---
src: ./pages/03-hooks.md
---
...
Blockquote with `✓ Check:` prefix. Phase 3's CSS renders blockquotes legibly against the locked palette. Mirrors Phase 1's pattern verbatim.
...
- **Phase 1 actual rendered files** — `pages/01-audience-setup/*.md` (read all 7 files) — ground-truth visual conventions Phase 3 mirrors. [VERIFIED — file inspection]
- **`slides.md`** — current orchestrator state, headmatter (lines 1-18), existing Phase 1 include (lines 28-30). [VERIFIED]
- **`style.css`** — Phase 3 CSS contract (auto-applied tinted panel + rust left rule on every Shiki block). [VERIFIED]
- **Phase 1 Plan 08 SUMMARY** — `.planning/phases/01-audience-setup/01-08-SUMMARY.md` — captures the build + spot-check pattern Plan 06 will mirror; also documents the live deviations (e.g., port-3030 stale-process gotcha) that Phase 3's build plan may want to anticipate. [VERIFIED]
