...
tags: [slidev, content, phase-3, section-02, hooks, ifttt, determinism]
requires:
  - pages/03-hooks.md (chapter orchestrator from 02-01, `src:` include for `02-why-hooks.md`)
  - .planning/phases/03-hooks/02-CONTEXT.md (D-02, D-03, D-04, D-12)
provides:
  - pages/03-hooks/02-why-hooks.md (Section 02 — Why hooks: IFTTT framing + 3 locked determinism quotes)
affects:
  - Chapter 3 narrative arc: HOOK-01 framing complete; Section 03 (how hooks work) can build on the IFTTT + determinism foundation
  - Phase 1 → Phase 3 bridge live on Slide 4 (`~/.claude/CLAUDE.md` callback to Win 2)
tech_stack_added: []
patterns_used:
...
key_files_created:
  - pages/03-hooks/02-why-hooks.md
key_files_modified: []
decisions: []
metrics:
  duration_minutes: 1
...
# Phase 3 Plan 2: Section 02 — Why hooks Summary

Authored Section 02 of Chapter 3: a 5-slide arc that frames why hooks exist — IFTTT framing for the non-technical participant (Slides 1-2), then three USER-LOCKED determinism quotes verbatim (Slides 3-5). Slide 4 carries the Phase 1 → Phase 3 bridge by calling back to the `~/.claude/CLAUDE.md` rule from Win 2 and framing it as the imperfect version of a hook. Single atomic commit per QUAL-02.

## What Was Built

- **pages/03-hooks/02-why-hooks.md** (NEW, 35 lines): One section file, no frontmatter, five slides separated by four mid-file `---` lines (each flanked by blank lines).
...
| Task | Hash    | Message                                                                       |
| ---- | ------- | ----------------------------------------------------------------------------- |
| 1    | 734de98 | feat(phase-3): section 02 why-hooks (IFTTT + locked determinism quotes)        |

## Verification

Plan's automated verification command passed (`VERIFY: PASSED`). All acceptance criteria confirmed:

- `pages/03-hooks/02-why-hooks.md` exists.
...
- **HOOK-01** — Chapter 3 frames why hooks exist via IFTTT framing + three determinism quotes; Phase 1 bridge present on Slide 4.
...
- File `pages/03-hooks/02-why-hooks.md` — FOUND.
