...
Chapter 3 of the v1.1 workshop deck. Teaches **what hooks are**, **how they work mechanically** (lifecycle events + `~/.claude/settings.json` wiring), and walks participants through **one small live hands-on task**: wiring `Stop` + `Notification` hooks to `afplay` (macOS) / `aplay` (Linux/WSL) so they hear a chime when Claude finishes a turn or asks for input.

Phase 3 ends when:
- `pages/03-hooks.md` is a `src:`-style chapter orchestrator including 5 section files under `pages/03-hooks/` (`01-cover.md` … `05-hands-on-fire.md`).
- `slides.md` `src:`-includes `pages/03-hooks.md`.
...
- **D-01:** Chapter 3 splits into 5 content sections + 1 closing build plan = **6 plans** for `/gsd-plan-phase 3`:
  - `pages/03-hooks/01-cover.md` — chapter cover slide (one-line provocation).
  ...
  - `pages/03-hooks/05-hands-on-fire.md` — participants trigger Claude, hear chimes, ✓ check (HOOK-03 fire).
...
- `.planning/REQUIREMENTS.md` — Phase 3 requirements: HOOK-01, HOOK-02, HOOK-03, QUAL-01, QUAL-02, QUAL-04.
- `.planning/ROADMAP.md` — Phase 3 goal + dependencies + per-phase research policy.
- `slides.md` — existing chapter orchestrator (preserve headmatter; add `src:` include for `pages/03-hooks.md`).

### Phase 1 artifacts (direct dependency)
- `.planning/phases/01-audience-setup/01-CONTEXT.md` — chapter-orchestrator pattern, ✓ check shape, tone discipline, slide-level conventions D-14..D-17 (carried forward as Phase 3 D-10..D-13).
...
- `.planning/phases/01-audience-setup/01-08-PLAN.md` — the Phase 1 closing `bunx slidev build` plan; Phase 3's plan 06 mirrors its shape.
- `pages/01-audience-setup.md` and `pages/01-audience-setup/*.md` — read at planning time to mirror Slidev frontmatter conventions, layout choices, and ✓ check rendering. Phase 3 must visually match.
...
- [Slidev — Slide layouts](https://sli.dev/builtin/layouts) — `default`, `section`, `two-cols` (if Section 03 needs side-by-side framing — Phase 3 currently does not).
...
### Reusable Assets
- `slides.md` — has v1.0 head matter (theme, fonts, Shiki, colorSchema) and a `src:` include for `pages/01-audience-setup.md`. Phase 3 appends one more `src:` line for `pages/03-hooks.md`. Does NOT modify head matter.
- `pages/01-audience-setup.md` — chapter orchestrator template Phase 3 mirrors at `pages/03-hooks.md`.
- `pages/01-audience-setup/0N-*.md` — section file templates Phase 3 mirrors at `pages/03-hooks/0N-*.md` (same frontmatter conventions, slide-separator style, ✓ Check rendering).
...
- **Chapter-file pattern (locked):** `slides.md` → `pages/03-hooks.md` → `pages/03-hooks/0N-*.md`. Identical to Phase 1.
- **Atomic section commits (QUAL-02):** one git commit per section file, scoped commit message (`feat(phase-3): section 02 why-hooks`, etc.).
...
- **✓ Check pattern:** Phase 1 Sections 05/06 use a copy-paste-compare check on a terminal-style block. Phase 3 Section 05 mirrors this shape (audio-cue check rather than visual but same callout style).

### Integration Points
- `slides.md` — Phase 3 adds a single `---\nsrc: ./pages/03-hooks.md\n---` block AFTER the existing Phase 1 include. Cover slide stays untouched.
- `pages/03-hooks.md` — NEW file; chapter orchestrator with `src:` includes for each section file.
- `pages/03-hooks/` — NEW directory; 5 section files (`01-cover.md` through `05-hands-on-fire.md`).
...
- **Pre/post relationship with skills + subagents** — how hooks interact with skill-loading and subagent lifecycles. Better placed after Chapter 2 (Skills) and Chapter 4 (Subagents) have established those primitives.
...
- **D-06 / D-08 superseded (2026-05-13, user-authorized):** Cross-OS guidance
  ...
  `pages/03-hooks/03-how-hooks-work.md`, `pages/03-hooks/04-hands-on-build.md`.
  Font-shrink tightened to `0.55rem` to fit three columns.
- **New section 04b added (2026-05-13, user-authorized):**
  `pages/03-hooks/04b-or-just-ask.md` demonstrates the natural-language
  ...
  Wired into orchestrator `pages/03-hooks.md` between 04 and 05. Chapter 3
