...
- **D-18:** Hooks chapter must cover: what hooks are (PreToolUse, PostToolUse, SessionStart, Stop, UserPromptSubmit — name all 5; mention 13 lifecycle events exist with a pointer to the source repo), the `settings.json` shape, one practical hook (PreToolUse block for `rm -rf` + `.env` access — chosen because it has the cleanest before/after demo), one defensive hook (concept only — e.g. PostToolUse logger), debugging when hooks don't fire.
- **D-18a (BEFORE/AFTER showcase — REQUIRED):** The Hooks chapter MUST include a clear **before/after** demonstration of the same task — *before the hook is wired*, then *after the hook is wired*. The recommended demo task: Claude is asked to clean up logs and reaches for `rm -rf logs/*`, OR Claude is asked about config and tries to `cat .env`. Show:
  1. **BEFORE slide:** terminal-style fenced block showing the dangerous command running unblocked (files gone / secrets exposed). Caption: "no hook — Claude does what it's told."
  2. **The hook source slide:** the `pre_tool_use.py` (or shell equivalent) script + the `.claude/settings.json` `hooks.PreToolUse` entry that wires it.
  3. **AFTER slide:** same prompt, same Claude, but the hook fires first — terminal shows the tool call BLOCKED (non-zero exit / `decision: "block"` JSON returned). Caption: "with the hook — Claude can't reach the dangerous primitive."
  4. **Mental model slide:** the lifecycle diagram showing where in the loop (between Claude → tool execution) the PreToolUse hook intercepts.
- **D-18b (source attribution):** The Hooks chapter draws on **disler/claude-code-hooks-mastery** (https://github.com/disler/claude-code-hooks-mastery) for: the 13-event lifecycle taxonomy, the PreToolUse block pattern for `rm -rf` and `.env`, and the `.claude/settings.json` `hooks` shape with `matcher` + `command`-type entries. Cite the repo on the recap slide as "further reading." Do NOT copy the repo's Python verbatim — adapt to short illustrative pseudo-code (≤25 lines per D-10). Keep attribution honest.
...
- **Hooks before/after concrete material (from disler/claude-code-hooks-mastery):** The repo's `pre_tool_use.py` blocks `rm -rf` (regex over `\brm\s+.*-[a-z]*r[a-z]*f`) AND any `.env` file access in Read/Edit/Write/Bash tools (allows `.env.sample`). Settings.json shape: `"hooks": { "PreToolUse": [ { "matcher": "", "hooks": [ { "type": "command", "command": "uv run .../pre_tool_use.py" } ] } ] }`. Use this exact shape in the chapter — it matches Claude Code's documented contract. The "BLOCKED" output the hook returns is exit code 2 with a JSON `{"decision": "block", "reason": "..."}` to stderr.
